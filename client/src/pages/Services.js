...
import { FaFigma, FaReact, FaMobileAlt, FaRedo, FaLayerGroup, FaUserShield, FaChartBar, FaCamera, FaStore, FaBullhorn, FaEnvelopeOpenText, FaDumbbell, FaBrain, FaFileInvoice, FaFire, FaCloud, FaGlobe, FaRocket, FaCheckCircle, FaGift } from 'react-icons/fa';
    ...
    <div className="pt-0">
              ...
              <tbody>
                {individualServices.map((srv, i) => (
                  <tr key={srv.name} className="border-b border-blue-50 dark:border-gray-800 hover:bg-blue-50/50 dark:hover:bg-gray-800/50 transition">
                    <td className="py-2 pr-2 font-semibold">{srv.name}</td>
                    <td className="py-2 pr-2 text-gray-600 dark:text-gray-300">{srv.desc}</td>
                    <td className="py-2 font-bold text-blue-600 dark:text-blue-400">{srv.price}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="py-16 bg-gradient-to-br from-blue-50 to-white dark:from-gray-800 dark:to-gray-900">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {usdPackages.map((pkg) => (
              <div key={pkg.name} className="relative rounded-2xl shadow-lg p-8 bg-white dark:bg-gray-800 border border-blue-100 dark:border-gray-700 flex flex-col items-center">
                ...
                <div className="text-3xl font-extrabold text-blue-600 dark:text-blue-400 mb-4">{pkg.price}</div>
                <ul className="mb-6 space-y-2">
                  {pkg.features.map((f, i) => (
                    <li key={i} className="flex items-center text-gray-700 dark:text-gray-200"><FaCheckCircle className="mr-2 text-green-500" /> {f}</li>
                  ))}
                </ul>
                <a href="/contact" className="btn-primary w-full text-center">Let's Build → Get Started</a>
              </div>
            ))}
          </div>
        </div>
              ...
              <tbody>
                {usdAddons.map((addon, i) => (
                  <tr key={addon.name} className="border-b border-blue-50 dark:border-gray-800 hover:bg-blue-50/50 dark:hover:bg-gray-800/50 transition">
                    <td className="py-2 pr-2 font-medium text-gray-700 dark:text-gray-200">{addon.name}</td>
                    <td className="py-2 font-bold text-blue-600 dark:text-blue-400">{addon.price}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="py-8 bg-white dark:bg-gray-900">
        <div className="container mx-auto px-4 max-w-3xl">
          <div className="rounded-xl p-6 bg-blue-50 dark:bg-gray-800 border border-blue-100 dark:border-gray-700">
            <h4 className="font-bold text-lg mb-2 flex items-center gap-2"><FaBrain className="text-blue-500" /> Pricing Strategy Logic (ROI-Based):</h4>
            <ul className="list-disc pl-6 text-gray-700 dark:text-gray-200 space-y-1">
              <li>Entry-level price = freelancer-level affordability to attract leads</li>
              <li>Mid-level pack = most value for small businesses/startups (high conversion)</li>
              <li>High-ticket plans = recurring or scalable for agencies / serial founders</li>
              <li>Add-ons = maximize average order value (AOV) without bloating base cost</li>
              <li>All packages are margin-buffered (30–50%) for agency ROI</li>
            </ul>
          </div>
        </div>
      </section>
